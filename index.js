...
function displayDataInTable(page = 1) {
    ...
    pageData.forEach((item, index) => {
        ...
        nameCell.textContent = item.firstname + " " + item.middlename + " " + item.lastname;
        ...
        editButton.className = "btn btn-info me-2 text-white";
        ...
        deleteButton.className = "btn btn-danger me-2 text-white";
        ...
        viewButton.className = "btn btn-success me-2 text-white";
    ...
    document.getElementById('page-info').textContent = `Page ${currentPage} of ${Math.ceil(arrPerson.length / itemsPerPage)}`;
    document.getElementById('prev-page').disabled = currentPage === 1;
    document.getElementById('next-page').disabled = currentPage === Math.ceil(arrPerson.length / itemsPerPage);
}
...
function emailUnique() {
    ...
    if (!isEmailUnique && !isContactUnique) {
        ...
        alert("This contact number is already in use. Please use a different contact number.");
    } else {
...
function addPerson() {
    ...
    if (newfirstname && newmiddlename && newlastname && newemail && newcontact && newp_add && newc_add && newcontactType && newdob) {
...
function deleteItem(index) {
    ...
    document.getElementById('delStatement').innerHTML = `Are you Sure You Want To Delete ?<h5>${arrPerson[deleteIndex].firstname + " " + arrPerson[deleteIndex].middlename + " " + arrPerson[deleteIndex].lastname + " "}</h5>`;
}
...
function filterItem(value) {
    const arrPerson = getStoredData();
    filteredArr = arrPerson.filter((e1) => e1.firstname.toLowerCase().includes(value.toLowerCase()) ||
...
function viewItem(index) {
    ...
    document.getElementById('v_fullname').innerHTML = "-  " + item.firstname + " " + item.middlename + " " + item.lastname;
    document.getElementById('v_email').innerHTML = "-  " + item.email;
    document.getElementById('v_contact-type').innerHTML = "-  " + item.contactType;
